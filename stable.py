...
class Platform:
    def __init__(self, x, y, width, height, platform_type="normal"):
        ...
        self.move_offset = random.uniform(0, 2 * math.pi)  # Für bewegliche Plattformen
        self.original_x = x
    
    def update(self, time_counter):
        if self.type == "moving":
            # Bewegliche Plattform
            self.rect.x = self.original_x + math.sin(time_counter * 0.05 + self.move_offset) * 50
...
def spawn_collectibles():
    """Spawne Münzen und Power-Ups auf Plattformen - weniger häufig"""
    for platform in platforms:
        ...
        if not has_nearby_coin and random.randint(1, 100) < 1:  # Nur 1% Chance für Münze (war 3%)
            coin_x = random.randint(platform.rect.x + 10, platform.rect.right - 10)
            coin_y = platform.rect.y - 20
            coins.append(Coin(coin_x, coin_y))
        
        if random.randint(1, 100) < 1:  # 1% Chance für Power-Up (bleibt gleich)
            power_x = random.randint(platform.rect.x + 10, platform.rect.right - 10)
            power_y = platform.rect.y - 25
